...
#==========================================
# init .tsv file
tsv_file_path = os.path.join(args.output_dir, 'dataser.tsv') 
...
for f in e_filelist:
    ...
    for bugtype in range(len(buglist)):
        ...
        for idx in range(len(grep_results)): 
            ...
            sed_cmd = "sed -i '" + line + "s" + sed_str(bug) + "' " + generate_file  
            
            print(sed_cmd)
            if args.make_dataset != None:
                shell(sed_cmd)
                description = re.sub(r'[\n\r]+', '', shell("diff "  + generate_file + ' ' + os.path.join(os.path.dirname(f), filename + '.v')))
